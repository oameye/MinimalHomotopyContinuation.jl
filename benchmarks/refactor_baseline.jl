...
function bench_small_total_degree()
    ...
    warm = _median_ns(
        @benchmarkable solve($prob, $alg; show_progress = false, threading = false)
    )
    steady = _median_ns(
        @benchmarkable solve($prob, $alg; show_progress = false, threading = false) samples = 50
    )
...
function bench_medium_polyhedral()
    ...
    steady = _median_ns(
        @benchmarkable solve($prob, $alg; show_progress = false, threading = false) samples = 30
    )
    return (warm = warm, steady = steady)
end
...
function run_refactor_baseline()
    ...
    println(
        "small_total_degree: warm=", _fmt_ms(small.warm), ", steady=", _fmt_ms(small.steady)
    )
    println(
    ...
    println(
        "parameter_homotopy: warm=", _fmt_ms(param.warm), ", steady=", _fmt_ms(param.steady)
    )
